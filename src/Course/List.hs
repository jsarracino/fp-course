...
filter ::
  ...
  -> List a
  -> List a
filter f = foldRight (\x a -> if f x then x :. a else a) Nil
...
flatten ((t :. ts) :. xs) = t :. (flatten $ ts :. xs)
...
seqOptional ::
  List (Optional a)
  -> Optional (List a)
seqOptional ((Full x) :. mxs) = case seqOptional mxs of 
  Full xs -> Full (x :. xs)
  Empty -> Empty
...
lengthGT4 ::
  List a
  -> Bool
lengthGT4 x = worker x 4
  where
    worker (_ :. ts) p 
      | p == 0 = True
      | p > 0  = worker ts (p - 1)
      | otherwise  = error "length worker called with bad counter"
    worker Nil _ = False
