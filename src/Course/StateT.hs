...
instance Monad f => Applicative (StateT s f) where
  ...
  (<*>) ::
   StateT s f (a -> b)
    ...
    -> StateT s f b
  stf <*> sta = StateT $ \s -> 
    (\(func, s') -> (\(a, s'') -> pure (func a, s'')) =<< runStateT sta s' ) =<< runStateT stf s
...
instance Monad f => Monad (StateT s f) where
  (=<<) ::
    ...
    -> StateT s f a
    -> StateT s f b
  func =<< sta = StateT $ \s -> (\(a, s') -> runStateT (func a) s' ) =<< runStateT sta s
...
distinct' ::
  ...
  List a
  -> List a
distinct' xs = eval' (filtering (\x -> state' $ \elems -> (S.notMember x elems, S.insert x elems)) xs) S.empty 
...
distinctF ::
  ...
  List a
  -> Optional (List a)
distinctF xs = evalT (filtering (\x -> StateT $ \elems -> if x > 100 then Empty else Full (S.notMember x elems, S.insert x elems)) xs) S.empty
...
instance Functor f => Functor (OptionalT f) where
  f <$> opt = OptionalT $ (\b -> mapOptional f b) <$> runOptionalT opt
...
instance Monad f => Applicative (OptionalT f) where
  ...
  -- fab <*> oa = OptionalT $ (\ofunc -> ofunc >>= (\func -> (\a -> mapOptional func a) =<< (runOptionalT oa))) =<< runOptionalT fab
  fab <*> oa = OptionalT $ builder =<< runOptionalT fab
    where
...
instance Monad f => Monad (OptionalT f) where
  ...
  f =<< oa = OptionalT $ worker =<< runOptionalT oa
    where
      worker Empty = pure Empty
      worker (Full x) = runOptionalT $ f x
...
instance Applicative (Logger l) where
  ...
  (Logger ll f) <*> (Logger lr x) = Logger (ll ++ lr) $ f x
...
instance Monad (Logger l) where
  ...
  lf =<< (Logger la x) = let (Logger lb y) = lf x in Logger (la ++ lb) y
