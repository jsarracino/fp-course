...
firstRepeat ::
  ...
  List a
  -> Optional a
firstRepeat xs = eval (findM (\x -> State $ \st -> (S.member x st, S.insert x st)) xs) S.empty
...
distinct ::
  ...
  List a
  -> List a
distinct xs = eval (filtering (\x -> State $ \st -> (S.notMember x st, S.insert x st)) xs) S.empty
